...
**Naming: full names (the `Magn` abbreviation was tried, then reverted).** Every card
name here fits within 18 tiles on its own, so `Magn` never *enabled* a name — it only
saved ~40 tiles of shared pool space, at the cost of ugly forms like "Dark Magn Girl".
Since the roster will carry **fewer total cards to save namespace** anyway (more
retirements), the ~40 tiles aren't worth the ugliness, so full names stand.
...
| | |
|---|---|
| **In-game name** | `Chronicle Magician` — 18 tiles |
| **ATK / DEF** | 2500 / 2500 |
| **Type** | Spellcaster |
...
| | |
|---|---|
| **In-game name** | `Dark Magician Girl` — 18 tiles |
| **ATK / DEF** | 2000 / 1700 |
| **Type** | Spellcaster |
...
|---|---|
| Cards recorded | **27** |
| Name tiles required | **420** (full names; `Magn` reverted) |
| Fusion-zone (#1–#300) slots required | **14** |
| Free-zone (#351–365) eligible | 13 (15 slots exist) |

### ⚠ The name pool must be paid for by retirements
...
length, so it will free less and fall short. The accepted plan is **retire more cards
than we add** (fewer total cards in the game) rather than relocate the pool. Roughly
30+ retirements gives comfortable headroom; the exact list is the next thing to build.
(Pool relocation stays a Project-2 option — bank 1's pool ends exactly at `$7FFF`, so
growing it needs 4480+ contiguous free bytes elsewhere and a re-pointed reader.)
